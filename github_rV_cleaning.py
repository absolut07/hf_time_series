...
#like there are 31 day in each month
...
#%%
"""same sitation for vairable vodostaj"""
...
ind=ind.iloc[::-1]
ind=ind.iloc[:5844]
dic={'padavine': padavine, 'temp_vazduha': temp, 'temp_vode': temp_vod, 'vodostaj': vod, 'time':ind}
...
df3=df3.drop([pd.Timestamp('2004-02-29 00:00:00'), pd.Timestamp('2008-02-29 00:00:00'),pd.Timestamp('2012-02-29 00:00:00'),pd.Timestamp('2016-02-29 00:00:00')])
...
#to english:
dff=df3.rename(columns={"padavine": "precipitation", "temp_vazduha": "air temperature",
'temp_vode':'water temperature', 'vodostaj': 'water level'})
...
#ttt=pd.DataFrame(y_plot_train.numpy())
#ttt=ttt.rename(columns={0:'model training data'})
#plot=df2.plot(ax=ax)#, ylim=(-1,50)
#plot=df4.plot(ax=ax)
#plot=df5.plot(ax=ax)
#plot=df5.plot(ax=ax)
#plot=df6.plot(ax=ax)
